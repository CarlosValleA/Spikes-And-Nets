...
% PoinCareF1(data,name,0,6);
function [y grafico] = PoinCareF(SavePdf,SaveFig,TimeFormat,data,name,mosaico,varargin)
warning off
visible_figure = 'off';
y = 0;
grafico = 0;
...
newname = strcat(name(1:end-4),'_PO');
...
if sum(size(partes))==2 & mosaico~=1
    ...
    if D==0
        title('Poincare Diagram '+string(name(1:end-4)))
...
%     save(newname+string('T'), '-struct', 'S0')
    ...
    if SavePdf
        saveas(figura,strcat(name(1:end-4),'_PO'),'pdf')
    end
end
...
if sum(size(partes))>2
    ...
    newname = strcat(name(1:end-4),'_PO',char(string(partes(2))));
%     S0.('PO') = T;
%     save(newname+string('T'), '-struct', 'S0')
...
%     p.FontWeight = 'bold';
    ...
    for i=1:partes(1)
        ...
        [a valores] =PoinCareF(SavePdf,SaveFig,TimeFormat,data((inicio:final)),name,0,6,i,length(data));
    ...
    newname = strcat(name(1:end-4),'_PO_time');
    if SaveFig
        savefig(newname)
    end
    if SavePdf
        saveas(panhandle,strcat(name(1:end-4),'_PO','time'),'pdf')
    end
    intervalos;
